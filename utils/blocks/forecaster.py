...
class Forecaster(nn.Module):
    def __init__(self, subnets, rnns):
        ...
        self.n_blocks = len(subnets)

        for index, (params, rnn) in enumerate(zip(subnets, rnns)):
            setattr(self, 'rnn' + str(self.n_blocks-index), rnn)
            setattr(self, 'stage' + str(self.n_blocks-index), make_layers(params))

    def forward_by_stage(self, input, state, subnet, rnn):
        input, state_stage = rnn(input, state, seq_len=cfg.BENCHMARK.OUT_LEN)
    ...
    def forward(self, hidden_states):
        ...
        for i in list(range(1, self.n_blocks))[::-1]:
